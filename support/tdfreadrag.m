...
%   $Revision: 1.1.8.3 $  $Date: 2010/10/08 17:26:51 $
...
% multiple underscores with one, and multiple line breaks with one (removes
% empty lines in the middle and allows empty lines at the end).  remove
% insignificant whitespace before and after delimiters or line breaks.  remove
% leading empty line.
if delimiter == tab
   matchexpr = {'\r\n' '([ _\n])\1+' ' *(\n|\t) *' '^\n'};
elseif delimiter == ' '
...
if length(delimitidx) ~= nobs*(nvars-1)
   warning('Short records detected. Fixed by padding.\n');
end
if nvars > 1
